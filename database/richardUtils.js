...
GuildMember.prototype.kayıtRolVer = function (rolidler = []) {
    let rol;
    if(this.roles.cache.has(roller.vip)) { 
    rol = this.roles.cache.clone().filter(e => e.managed).map(e => e.id).concat(rolidler).concat(roller.vip) 
    } else {
...
Guild.prototype.log = async function log(cezano, user, admin, tip, channelName) {
    ...
    if (channel) {
        let embed = new MessageEmbed()
          .setAuthor(ayarlar.embed.başlık, client.guilds.cache.get(genel.server).iconURL({dynamic: true, size: 2048})).setColor(ayarlar.embed.renk)
